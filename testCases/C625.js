...
async function C625() {
  try {
    await testBase('C625_Roles page shows a summary of Admins, Editors, and Viewers.', async (driver) => {
      ...
      let vars = {}; // Container for storing test variables

      // 🛠 Step 1: Configure the browser window and login as admin
      await windowConfiguration(driver, "UMS");
      const serviceEnv = await windowConfiguration(driver, "UMS"); // Determines flow based on environment (PROD/DEV)

      await loginAdmin(driver, vars);

      // 🔄 Step 2: Ensure organization and user name reset
      await resetToOriginalUserNameInRoothOrganization(driver);

      // 📌 Step 3: Navigate to User Management & Roles page
      ...
      const roleSummaryXPaths = (serviceEnv === 'PROD')
        ? {
      ...
      vars.numberOfEditors = await extractRoleCount(driver, "//div[2]/div/pc-input-checkbox/flex-col/mat-checkbox/div/label");
      vars.numberOfViewers = await extractRoleCount(driver, "//div[3]/div/pc-input-checkbox/flex-col/mat-checkbox/div/label");
...
async function extractRoleCount(driver, xpath) {
  ...
  return parseInt(elementText.match(/\((\d+)\)/)[1], 10); // Extracts number inside parentheses
}
